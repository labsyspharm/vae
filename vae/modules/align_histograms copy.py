    ...
    if not os.path.exists(
      os.path.join(config.output_path, 'checkpoints/ALIGN_HISTOGRAMS.txt')):
        ...
        for e, (sample, path) in enumerate(zarrs.items()):
            ...
            if not os.path.exists(
              os.path.join(save_dir, 'ref_limits.pkl')):
                print(
                    f'Storing channel limits for sample {sample}'
                )
                # non_zero_mask = da.where(
                #     z > thresholds[(sample, marker)], z, float('inf')
                # )
                # channel_mins = non_zero_mask.min(axis=(1, 2, 3)).compute()
                channel_mins = [
                    value for (key, value) in thresholds.items() if 
                    key[0] == sample
                ]
                # channel_maxs = z.max(axis=(1, 2, 3)).compute()
                keys = [
                    i for i in zip([sample]*len(config.tif_channels), 
                                   config.tif_channels)
                ]
                vals = [
                    i for i in zip(channel_mins, 
                                   [65535]*len(config.tif_channels))
                ]
                for key, val in zip(keys, vals):
                    ref_limits[key] = val   
            
        if not os.path.exists(os.path.join(save_dir, 'ref_limits.pkl')):
            with open(
                  os.path.join(
                   save_dir, 'ref_limits.pkl'), 'wb') as handle:
                pickle.dump(
                        ref_limits, handle, 
            ...
            with open(os.path.join(
               save_dir, 'ref_limits.pkl'), 'rb') as handle:
                ref_limits = pickle.load(handle)

        #######################################################################
        # generate dataframe of anchor points
        
        max_pixels = 2_000_000
        if not os.path.exists(os.path.join(save_dir, 'anchors.csv')):
            column_index = pd.MultiIndex.from_tuples(
                imgs.keys(), names=['sample', 'channel']
            )
            anchors = pd.DataFrame(
                index=list(set(item for tup in scalings for item in tup)), 
                columns=column_index, dtype=np.float64
            )
            for (sample, marker) in imgs.keys():
                print(f'Computing anchor points for {sample}, {marker}')
                raveled_data = ravel_data(
                    channel_patches=imgs[(sample, marker)],
                    threshold=thresholds[(sample, marker)], 
                    max_pixels=max_pixels
                )
                anch_out = compute_anchors(raveled_data=raveled_data)
                anchors.loc[:, (sample, marker)] = pd.Series(anch_out)
            anchors.to_csv(os.path.join(save_dir, 'anchors.csv'))
        else:
            anchors = pd.read_csv(
                os.path.join(save_dir, 'anchors.csv'), 
                index_col=0, header=[0, 1]
            )
        print()
        
        #######################################################################
        # compute linear transformations (slope & offset) between anchor points
        samples = anchors.columns.get_level_values(0).unique()
        if not os.path.exists(os.path.join(save_dir, 'scaling_functions.pkl')):
            scaling_funcs = {}
            ref_anch = anchors.loc[:, samples[0]]
            for sample in samples:
                anch_data = anchors.loc[:, sample]
                for marker in config.tif_channels:
                    scaling_dict = {}
                    norm_fn = Poly.fit(
                        [np.log(ref_limits[marker][0] + 1), 
                         np.log(ref_limits[marker][1] + 1)], 
                        [0, 1], deg=1
                    )
                    for scale in scalings:
                        print(
                            f'Computing {scale} transformation for ' 
                            f'{sample}, {marker}'
                        )
                        poly1d_fn = Poly.fit(
                            anch_data.loc[list(scale), marker], 
                            ref_anch.loc[list(scale), marker], 
                            deg=1
                        )
                        poly1d_fn = poly1d_fn  # norm_fn() if needed
                        scaling_dict[scale] = poly1d_fn
                    scaling_funcs[(sample, marker)] = scaling_dict
            with open(
              os.path.join(save_dir, 'scaling_functions.pkl'), 'wb') as handle:
                pickle.dump(
                    scaling_funcs, handle, 
                    protocol=pickle.HIGHEST_PROTOCOL
                )
        else:
            with open(
              os.path.join(save_dir, 'scaling_functions.pkl'), 'rb') as handle:
                scaling_funcs = pickle.load(handle)
        print()
        
        #######################################################################
        # vizualize transformed histograms

        for scale in scalings + []:  # 'raw'
            print(f'Generating channel histograms plot for {scale}')
            ...
            for marker, ax in zip(config.tif_channels, axs.ravel()):
                handles = []
                if marker in config.tif_channels:  # for quick debugging
                    ...
                    for sample, color in zip(samples, colors):
                        ...
                        if scale == 'raw':
                            ...
                            non_zero = np.where(raveled_data > 0)
                            min_val, max_val = ref_limits[(sample, marker)]
                            min_val = np.log10(min_val + 1)
                            max_val = np.log10(max_val + 1)
                            raveled_data[non_zero] -= min_val
                            raveled_data[non_zero] /= (max_val - min_val)
                            raveled_data = raveled_data[raveled_data > 0]
                            raveled_data = np.clip(raveled_data, 0, 1)
                            # data_transformed = scaling_funcs[
                            #     (sample, marker)][scale](raveled_data)
                        ...
                        fig.suptitle(scale, fontsize=12)
