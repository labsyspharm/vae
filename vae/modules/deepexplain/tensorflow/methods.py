...
def original_grad(op, grad):
    ...
    :return: Tensor
    """
    if op.type not in SUPPORTED_ACTIVATIONS + SUPPORTED_OPERATIONS:
...
class RectifiedGradientMod(GradientBasedMethod):
    ...
    def nonlinearity_grad_override(cls, op, grad):
        ...
        if 'Logits' in op.name:
            ...
            activation_grad = tf.abs(op.outputs[0] * grad)
...
class RectifiedGradientConst(GradientBasedMethod):
    t = None
...
"""
Rectified Gradient with Proportional Redistribution Rule
"""
...
attribution_methods = OrderedDict({
    'zero': (DummyZero, 0),
    'rectgrad': (RectifiedGradient, 1),
    'rectgradprr': (RectifiedGradientPRR, 2),
    'rectgradconst': (RectifiedGradientConst, 3),
    'saliency': (Saliency, 4),
    'grad*input': (GradientXInput, 5),
    'deconv': (Deconvolution, 6),
    'guidedbp': (GuidedBackpropagation, 7),
...
@ops.RegisterGradient("PaddingTrick")
...
class DeepExplain(object):
    ...
    def get_override_map():
        gmap = dict((a, 'DeepExplainGrad') for a in SUPPORTED_ACTIVATIONS)
        gmap['Conv2D'] = 'PaddingTrick'
        gmap['MaxPool'] = 'PRR'
        return gmap
